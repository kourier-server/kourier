...
ARG THREAD_COUNT
ARG QT_VERSION
ENV INSTALL_ROOT_DIR="/opt/install-root"
...
RUN --mount=target=/var/lib/apt/lists,type=cache \
    ...
    echo "export CFLAGS=\"-Wno-unused-command-line-argument -fPIC -fno-omit-frame-pointer\"" >> /opt/env-vars.sh; \
    echo "export CXXFLAGS=\"-Wno-unused-command-line-argument -fPIC -fno-omit-frame-pointer -stdlib=libc++ -std=c++20 -stdlib++-isystem ${LLVM_INSTALLATION_DIR}/include -stdlib++-isystem ${LLVM_INSTALLATION_DIR}/include/c++/v1 -stdlib++-isystem ${LLVM_INSTALLATION_DIR}/include/${LLVM_HOST_TARGET}/c++/v1\"" >> /opt/env-vars.sh; \
    echo "export LDFLAGS=\"-Wno-unused-command-line-argument -fno-omit-frame-pointer -rtlib=compiler-rt -stdlib=libc++ -Wl,-rpath,\"${LLVM_INSTALLATION_DIR}/lib\" -Wl,-rpath,\"${LLVM_INSTALLATION_DIR}/lib/${LLVM_HOST_TARGET}\" -Wl,-rpath,\"${LLVM_INSTALLATION_DIR}/lib/clang/${LLVM_MAJOR_VERSION}/lib/${LLVM_HOST_TARGET}\" -L\"${LLVM_INSTALLATION_DIR}/lib\" -L\"${LLVM_INSTALLATION_DIR}/lib/${LLVM_HOST_TARGET}\" -L\"${LLVM_INSTALLATION_DIR}/lib/clang/${LLVM_MAJOR_VERSION}/lib/${LLVM_HOST_TARGET}\" -Wl,-rpath,\"${INSTALL_ROOT_DIR}/lib\" -Wl,-rpath,\"${INSTALL_ROOT_DIR}/lib64\" -Wl,-rpath,\"${INSTALL_ROOT_DIR}/lib/${SYS_TARGET_TRIPLET}\" -L\"${INSTALL_ROOT_DIR}/lib\" -L\"${INSTALL_ROOT_DIR}/lib64\" -L\"${INSTALL_ROOT_DIR}/lib/${SYS_TARGET_TRIPLET}\"\"" >> /opt/env-vars.sh; \
    LIBRARY_PATH="${LLVM_INSTALLATION_DIR}/lib:${LLVM_INSTALLATION_DIR}/lib/${LLVM_HOST_TARGET}:${LLVM_INSTALLATION_DIR}/lib/clang/${LLVM_MAJOR_VERSION}/lib/${LLVM_HOST_TARGET}:${INSTALL_ROOT_DIR}/lib:${INSTALL_ROOT_DIR}/lib64:${INSTALL_ROOT_DIR}/lib/${SYS_TARGET_TRIPLET}:$LD_LIBRARY_PATH"; \
    ...
    CMAKE_PREFIX_PATH="${LLVM_INSTALLATION_DIR}:${LLVM_INSTALLATION_DIR}/lib/clang/${LLVM_MAJOR_VERSION}:${INSTALL_ROOT_DIR}:${CMAKE_PREFIX_PATH}"; \
...
RUN --mount=target=/var/lib/apt/lists,type=cache \
    ...
    cd lz4-${LZ4_VERSION}; \
    make -j${THREAD_COUNT}; \
    make install PREFIX=${INSTALL_ROOT_DIR}; \
    ...
    mkdir build; \
    cd build; \
    cmake -G Ninja .. -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release"; \
    ../configure; \
    ...
    cp $(ls libbz2*.so*) ${INSTALL_ROOT_DIR}/lib; \
    ...
    mkdir build; \
    cd build; \
    cmake -G Ninja ../pcre2-${PCRE2_VERSION} -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release" -DPCRE2_SUPPORT_JIT="ON" -DPCRE2_SUPPORT_UNICODE="ON" -DBUILD_SHARED_LIBS="ON" -DPCRE2_BUILD_PCRE2_8="ON" -DPCRE2_BUILD_PCRE2_16="ON" -DPCRE2_BUILD_PCRE2_32="ON"; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
    ...
    mkdir build; \
    cd build; \
    cmake -G Ninja ../google-brotli-${BROTLI_VERSION} -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release"; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
    ...
    mkdir build; \
    cd build; \
    cmake -G Ninja ../google-draco-${DRACO_VERSION} -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release" -DBUILD_SHARED_LIBS="ON"; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
    ...
    mkdir build; \
    cd build; \
    cmake -G Ninja ../pugixml-${PUGIXML_VERSION} -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release" -DBUILD_SHARED_LIBS="ON"; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
    ...
    mkdir build; \
    cd build; \
    CXXFLAGS="-Wno-nontrivial-memcall $CXXFLAGS" cmake -G Ninja ../assimp -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release" -DASSIMP_BUILD_TESTS="OFF"; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
    ...
    mkdir build; \
    cd build; \
    ../openssl-${OPENSSL_VERSION}/Configure --prefix=${INSTALL_ROOT_DIR} --release -shared no-tests no-ssl3 no-comp; \
    make -j${THREAD_COUNT}; \
    make install; \
    hash -r; \
    ...
    mkdir build; \
    cd build; \
    ../source/configure --prefix=${INSTALL_ROOT_DIR} --enable-release --enable-shared --with-data-packaging=library; \
    make -j${THREAD_COUNT}; \
    make install; \
    ...
    mkdir build; \
    cd build; \
    CFLAGS="-Wno-implicit-function-declaration $CFLAGS" CXXFLAGS="-Qunused-arguments $CXXFLAGS" meson setup ../glib-${GLIB2_VERSION} -Dbuildtype='release' -Dcpp_stdlib='libc++' -Db_lundef='false' -Dwerror='false' --prefix=${INSTALL_ROOT_DIR}; \
    meson compile; \
    meson install; \
    ...
    mkdir build; \
    cd build; \
    cmake -G Ninja ../dbus-${DBUS_VERSION} -DCMAKE_INSTALL_PREFIX="${INSTALL_ROOT_DIR}" -DCMAKE_BUILD_TYPE="Release" -DDBUS_ENABLE_VERBOSE_MODE="OFF" -DDBUS_DISABLE_ASSERT="ON" -DDBUS_BUILD_TESTS="OFF"; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
    ...
    mkdir build; \
    cd build; \
    ../krb5-${KERBEROS_VERSION}/src/configure --prefix=${INSTALL_ROOT_DIR}; \
    make -j${THREAD_COUNT}; \
    make install; \
    rm -rf $(ls -A /var/lib/apt/lists/); \
    rm -rf $(ls -A /var/cache/apt/); \
...
RUN --mount=target=/var/lib/apt/lists,type=cache \
    ...
    mkdir build; \
    cd build; \
    ../qtbase/configure -prefix ${INSTALL_ROOT_DIR} -platform linux-clang-libc++ -release -shared -force-bundled-libs -system-zlib -system-pcre -qt-sqlite -no-gui -no-widgets -nomake examples -nomake tests -nomake benchmarks -- -DCMAKE_CXX_STANDARD=20 -DCMAKE_JOB_POOLS:STRING=link_jobs=1 -DCMAKE_JOB_POOL_LINK:STRING=link_jobs; \
    cmake --build . --parallel ${THREAD_COUNT}; \
    cmake --install .; \
