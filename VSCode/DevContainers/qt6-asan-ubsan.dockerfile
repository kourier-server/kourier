...
FROM vscode-dev-container:cpp-asan-ubsan

ARG THREAD_COUNT
ARG QT_VERSION
ENV SANITIZED_INSTALL_ROOT_DIR="/opt/root-sanitized"
# We disable leak detection because it requires SYS_PTRACE capability,
# which we don't have during build. We also disable halting on error
# to prevent build/tools from failing.
ENV ASAN_OPTIONS=halt_on_error=0:exitcode=0:detect_leaks=0
ENV UBSAN_OPTIONS=halt_on_error=0:exitcode=0:print_stacktrace=0
...
RUN --mount=target=/var/lib/apt/lists,type=cache \
    ...
    mkdir build; \
    cd build; \
    CFLAGS="-Wno-implicit-function-declaration $CFLAGS" CXXFLAGS="-Qunused-arguments $CXXFLAGS" meson setup ../glib-${GLIB2_VERSION} -Dbuildtype='release' -Dcpp_stdlib='libc++' -Db_lundef='false' -Db_sanitize='address,undefined' -Dwerror='false' --prefix=${SANITIZED_INSTALL_ROOT_DIR}; \
    meson compile; \
    meson install; \
...
ENV ASAN_OPTIONS=halt_on_error=0:exitcode=0:detect_leaks=1:strict_string_checks=1:detect_stack_use_after_return=1:check_initialization_order=1
ENV UBSAN_OPTIONS=halt_on_error=0:exitcode=0:print_stacktrace=1
